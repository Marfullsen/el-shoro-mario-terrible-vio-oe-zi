...
class Game(object):
    ...
    def __init__(self, screen, continuing=False):
        ...
        if not continuing:
            cutscene(self.screen,
                     ['Nota: Esto solo es una version de prueba',
                      'del juego, no esperih tanto',
                      'apenah hay cuatro niveleh.'])
            stop_music()
    ...
    def intro_level(self):
        ...
        self.draw_stats()
        ren = self.font.render("Nivel 1-%d" % self.lvl, 1, (255, 255, 255))
        self.screen.blit(ren, (320-ren.get_width()/2, 230))
        ren = self.font.render("Vias x%d" % self.lives, 1, (255, 255, 255))
    ...
    def next_level(self):
        ...
        try:
            ...
            if self.lives == 0: # Fix =)
                self.lives += 1   
            cutscene(self.screen,
            ['Esto solo es una version de prueba',
             'Asih que no wei poh',
             'apreta enter pa irse a la chucha.'])
    ...
    def show_death(self):
        ren = self.font.render("Aweonao", 1, (0, 0, 0))
        self.screen.blit(ren, (320-ren.get_width()/5, 235))
        pygame.display.flip()
        pygame.time.wait(2500)
...
#_______________________________________________________________________
    ...
    def draw_stats(self):
        ...
        if lives < 0:
            lives = 0
        ren = self.font.render("Achuntacion: %05d" % self.score, 1, (255, 255, 255))
        self.screen.blit(ren, (640-ren.get_width(), 16))
        ren = self.font.render("x%d" % lives, 1, (255, 255, 255))
        self.screen.blit(ren, (285+04, 24))
        ren = self.font.render("Nivel 1-%0d" % self.lvl, 1, (255, 255, 255))
        self.screen.blit(ren, (245-ren.get_width(), 16))
        ren = self.font.render("Weas(no pescar): %d" % self.clock.get_fps(), 1, (255, 255, 255))
        self.screen.blit(ren, (311, 41))
        ren = self.font.render("Puntoh:%05d" % self.highscore, 1, (255, 255, 255))
        self.screen.blit(ren, (260-ren.get_width(), 38))
        ren1 = self.font.render("Tiempo: %d" % self.time, 1, (255, 255, 255))
        ren2 = self.font.render("Tiempo: %d" % self.time, 1, Color("#ffffff"))
        self.screen.blit(ren1, (465, 60))
        self.screen.blit(ren2, (465, 60))
